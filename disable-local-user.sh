...
echo '	-r Removes the home direcotry associated with the account(s)'>&2
...
if [["${UID}" -ne 0]]
then 
  echo 'Please run with sudo or superuser privileges.' >&2
  exit 1
fi
...
#Loop through all the usernames e=supplied as arguments
for USERNAME in "${@}"
do 
  echo "Processing user: ${USERNMAE}"
...
#Create and archive if requested to do so. 
if [[ "${ARCHIVE}" = 'true' ]] 
then 
...
if [[ ! -d "${ARCHIVE_DIR}" ]] 
then 
 echo "Creating ${ARHIVE_DIR} directory." 
 mkdir -p ${ARCHIVE_DIR}
 if [["${?}" -ne 0 ]] 
 then
  echo "The arhcive directory ${ARCHIVE_DIR} could not be created.">&2
  exit 1
 fi 
fi 
...
if [[ -d "${HOME_DIR}" ]] 
then
  echo "Archiving ${HOME_DIR} to ${ARCHIVE_FLE}"
